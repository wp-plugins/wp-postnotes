<?php
class dc_wp_widget_1_2_0 extends dc_base_1_0_2 {
	var $max_widgets = 20;
	var $Description = "";
	function init()
	{
	...
	function config($name,$callback,$option_callback = null,$dims = null,$Desc = null)
	{
		...
		add_action('sidebar_admin_setup', array(&$this,'register'));
	}
	...
	function register() {
		...
		static $registered;
		if ($registered || empty($this->Base))
			return;
		$o=$this->loadClass('wp_options');
		if ( !$option = $o->values[$this->Name] )
			$option = array();
		...
		$name = $this->Name;
		$registered = false;
		foreach ( array_keys($option) as $o ) {
			if (!empty($o))
		{
				$id = $this->Base."-$o"; 
